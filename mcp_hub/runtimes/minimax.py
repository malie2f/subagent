"""MiniMax Code runtime adapter —— 当前是 STUB，CLI 入口未就绪。

状态（2026-07-18 实测）：
    - MiniMax Code 是 Electron 桌面 app，装在 D:\\MiniMax Code\\
    - C:\\Users\\Lenovo\\.mavis\\bin\\mavis.cmd 和 minimax.cmd 是两个完全相同的 stub，
      都指向 D:\\MiniMax Code\\resources\\resources\\daemon\\cli.js
    - **该路径不存在**（resources\\resources\\ 下只有 Mac 安装包图标之类）
    - 调 `mavis --help` / `minimax --help` 都会报 Cannot find module

什么时候这个 stub 可以变实：
    1) MiniMax Code 桌面 app 内部真的把 daemon 暴露到可访问路径（如
       D:\\MiniMax Code\\resources\\app.asar 里 unpacked 出的 cli.js），且
    2) 那个 daemon 接受同样的非交互调用方式（task / workdir / model）

届时只需把 _BINARY 和 _EXTRA_ARGS 改对，spawn 就能跑起来。

为什么还占个 stub 不直接删：
    - 用户的 mcp-hub 设计原则是 agent-agnostic，runtimes/ 目录里列齐所有
      已知的潜在 agent 更符合"任何 coding agent 都能挂上来"的初衷
    - 占位也方便 MiniMax Code 修好 CLI 后直接填空
...
class MinimaxAdapter(RuntimeAdapter):
    name = "minimax"
    binary = "minimax"

    # 候选 binary 名（按顺序探测）
    _CANDIDATE_BINARIES = ("minimax", "minimax-code", "minimax_code")
    ...
    def list_models(self) -> list[str]:
        """minimax code 内部用的是 mavis daemon，支持的模型由 daemon 决定。

        等 CLI 修好后再通过 `minimax models` 之类的子命令拿真实列表。
        现在返回几个常用的猜测值（MiniMax M 系列）。
        """
        return [
            "minimax-M3",
            "minimax-M2.7",
    ...
    async def spawn(
        ...
        """STUB：当前 minimax CLI wrapper 路径坏了，spawn 出来的子进程会立刻挂。
    ...
    async def wait(self, handle: SubagentHandle, timeout_sec: int) -> SubagentResult:
        ...
        return SubagentResult(
            ...
            artifacts=[],
            error=(
                "minimax CLI 当前不可用：桌面 app 的 daemon 入口路径不存在。"
                "请等待 MiniMax Code 修复 CLI wrapper，"
                "或编辑 mcp_hub/runtimes/minimax.py 把 _resolve_cmd() 改成实际能跑的入口。"
            ),
        )
    ...
    def info(self) -> dict:
        ...
        d["status"] = "stub"
        d["note"] = (
            "MiniMax Code 桌面 app 当前未暴露可用的 CLI 入口。"

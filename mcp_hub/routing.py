    ...
    "scenarios": {
        ...
        "complex_reasoning": {
            ...
            "models": {
                ...
                "fast": ("opencode", "deepseek/deepseek-v4-flash"),
            },
        },
        "vision": {
            "name": "图像 / 视频 / 截图 / 多模态理解",
        ...
        "deepseek": {
            ...
            "models": {
                ...
                "balanced": ("opencode", "deepseek/deepseek-v4-flash"),
...
- antigravity: 本机 Gemini CLI 已登录的模型（适合 coding / 看图 / 快速问答）
- codex: 本机 Codex CLI 已登录的模型（适合复杂算法 / 兜底）
- claude: 本机 Claude Code CLI 已登录的模型
- opencode: 本机 OpenCode 已配置的 provider/model（deepseek/* 等）
- qoder / kimi / 其它：仅当该 runtime 已在仪表盘连接且 CLI 可用

选择规则：
1. 只选列表里真实存在的 runtime + model，不要编造个人中转或号池名。
2. coding 任务优先已连接的 antigravity 或 opencode。
3. 复杂推理优先 codex 或 claude。
4. 提到 deepseek 时优先 deepseek/deepseek-v4-pro，其次 deepseek/deepseek-v4-flash。
5. 中文长文本优先 kimi。
6. 图像/视频优先 Gemini。

"""CodeBuddy CLI runtime adapter —— 包装 `codebuddy -p --permission-mode auto ...`。

实测命令（CodeBuddy Code）：
    codebuddy -p --permission-mode auto --model deepseek-v3-2-volc "task..."

关键参数：
    -p / --print            非交互模式，结果打到 stdout
    --permission-mode auto  自动批准所有工具调用（subagent 必须）
    --model <model>         选模型（deepseek-v3-2-volc / glm-5.0 / kimi-k2.5 等）
    --add-dir <dir>         允许子 agent 访问的工作目录（可多次给）
    --session-id <uuid>     指定 session id（落盘，可续跑）
    --resume <uuid>         续跑已有 session

注意：
    - codebuddy 默认会进交互 REPL，**必须**给 -p
    - 不给 --permission-mode auto 会卡在权限确认（hang）
    - workdir 用绝对路径；resume 必须在同一 workdir
"""
...
class CodebuddyAdapter(RuntimeAdapter):
    name = "codebuddy"
    binary = "codebuddy"
    supports_resume = True

    # codebuddy 实际可用模型 ID（来自 codebuddy 模型选择界面的括号内 ID）
    DEFAULT_MODELS = [
        "hy3",
        "glm-5.2",
        "glm-5.1",
    ...
    def _session_uuid(task_id: str) -> str | None:
        """task_id（12 位 hex，可能带续跑后缀 rsN）→ 固定 UUID，当 codebuddy session id 用。"""
    ...
    def list_models(self) -> list[str]:
        """codebuddy 没有 --list-models，先返回 help 里公布的模型列表。"""
        return list(self.DEFAULT_MODELS)

    def _parse_model(self, model: str) -> str:
        """支持 'codebuddy/deepseek-v3-2-volc' 和裸 'deepseek-v3-2-volc' 两种写法。"""
        prefix = f"{self.name}/"
    ...
    async def spawn(
        ...
        return await self._launch(task_id, model, task, workdir, session_args=[])

    def extract_session_id(self, handle: SubagentHandle) -> str | None:
        """session id 由 task_id 确定性推出。"""
    ...
    async def resume_spawn(
        ...
        return await self._launch(
            task_id, model, task, workdir, session_args=["--resume", session_id]
        )
    ...
    async def _launch(
        ...
        cmd = [
            *cmd_prefix,
            ...
            "--model", model_name,    # 选模型（去掉 codebuddy/ 前缀）
        ]
        if sid:
            cmd += ["--session-id", sid]
        cmd += session_args          # --resume <uuid>（续跑时）
...
def parse_live_log(stdout: str, stderr: str = "") -> list[dict[str, Any]]:
    """运行中的 codebuddy 子 agent：从当前 stdout 实时解析事件预览。"""
